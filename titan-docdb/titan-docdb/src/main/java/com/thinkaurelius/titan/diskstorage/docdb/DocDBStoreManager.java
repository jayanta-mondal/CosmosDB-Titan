...
public class DocDBStoreManager extends LocalStoreManager implements OrderedKeyValueStoreManager {

    //private static final Logger log = LoggerFactory.getLogger(DocDBStoreManager.class);

    public static final ConfigNamespace BERKELEY_NS =
            new ConfigNamespace(GraphDatabaseConfiguration.STORAGE_NS, "docdb", "DocDB configuration options");
    ...
    private final Map<String, DocDBKeyValueStore> stores;
    ...
    private static final com.microsoft.azure.documentdb.DocumentClient documentClient = new DocumentClient(END_POINT, MASTER_KEY, ConnectionPolicy.GetDefault(), ConsistencyLevel.Session);

    public DocDBStoreManager(Configuration configuration) throws BackendException {
        super(configuration);
        stores = new HashMap<String, DocDBKeyValueStore>();
...
//        features.supportsMultiQuery = false;
    ...
    private void initialize(int cachePercent) throws BackendException {
        try {
            ...
            throw new PermanentBackendException("Error during DocDB initialization: ", e);
        }
    ...
    @Override
    public DocDBTx beginTransaction(final BaseTransactionConfig txCfg) throws BackendException {
        try {
            Transaction tx = null;
            ...
            DocDBTx btx = new DocDBTx(tx, ConfigOption.getEnumValue(effectiveCfg.get(LOCK_MODE),LockMode.class), txCfg);

            //if (log.isTraceEnabled()) {
             //   log.trace("DocDB tx created", new TransactionBegin(btx.toString()));
            //}

            return btx;
        } catch (DatabaseException e) {
            throw new PermanentBackendException("Could not start DocDB transaction", e);
        }
    }

    @Override
    public DocDBKeyValueStore openDatabase(String name) throws BackendException {
        Preconditions.checkNotNull(name);
        if (stores.containsKey(name)) {
            DocDBKeyValueStore store = stores.get(name);
            return store;
        }
        try {
            ...
            com.sleepycat.je.Database db = environment.openDatabase(null, name, dbConfig);

            //log.debug("Opened database {}", name, new Throwable());

            DocDBKeyValueStore store = new DocDBKeyValueStore(name, db, this, documentClient);
            stores.put(name, store);
            return store;
        } catch (DatabaseException e) {
            throw new PermanentBackendException("Could not open DocDB data store", e);
        }
    }
    ...
    public void mutateMany(Map<String, KVMutation> mutations, StoreTransaction txh) throws BackendException {
        for (Map.Entry<String,KVMutation> muts : mutations.entrySet()) {
            DocDBKeyValueStore store = openDatabase(muts.getKey());
    ...
    void removeDatabase(DocDBKeyValueStore db) {
    ...
    public void close() throws BackendException {
        if (environment != null) {
            ...
            try {
                ...
                throw new PermanentBackendException("Could not close DocDB database", e);
            }
        }
